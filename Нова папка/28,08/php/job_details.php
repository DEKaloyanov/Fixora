...
require_once __DIR__ . '/label_utils.php'; // ⬅️
session_start();
...
function render_payment_chips(array $job): string {
  ...
  if (!empty($pm['custom']) && is_array($pm['custom'])) {
    foreach ($pm['custom'] as $c) {
      $lbl = htmlspecialchars((string)($c['label'] ?? 'Друго'));
      $pr  = htmlspecialchars((string)($c['price'] ?? ''));
      if ($pr !== '') $out[] = '<span class="jd-chip"><i class="fas fa-tag"></i>' . $lbl . ': <strong>' . $pr . '</strong></span>';
    }
  }
...
<head>
  <meta charset="UTF-8">
  <title><?= esc($profLbl) ?> — Детайли за обявата | Fixora-Build</title>
    ...
    <aside class="jd-side">
      ...
      <?php if (!empty($job['work_status']) && $job['work_status']==='team' && !empty($job['team_size'])): ?>
        ...
        <?php
          ...
          if ($members) {
            echo '<p><strong>Имена:</strong> ' . esc(implode(', ', $members)) . '</p>';
          }
        ?>
      <?php endif; ?>
