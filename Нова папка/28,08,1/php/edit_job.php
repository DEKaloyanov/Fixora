...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($finalOrder) {
        foreach ($finalOrder as $tok) {
            if (isset($tok['t']) && $tok['t'] === 'e' && !empty($tok['p'])) {
                if (in_array($tok['p'], $oldImages, true)) $resultImages[] = $tok['p'];
    ...
    // Изтриване от диска: вече неползвани стари
    ...
    if (jobs_has_col($conn,'title'))  $set['title']  = ($title!=='' ? $title : null);
    if (jobs_has_col($conn,'region')) $set['region'] = ($region!=='' ? $region : null);

    $parts = []; $params = [];
    foreach ($set as $k=>$v){ $parts[] = "$k = :$k"; $params[":$k"] = $v; }
    $params[':id'] = $job_id; $params[':user_id']=$user_id;

    $sql = "UPDATE jobs SET ".implode(', ',$parts)." WHERE id = :id AND user_id = :user_id";
...
<body>
    <main class="job-form-container">
        ...
        <form method="POST" id="editJobForm" enctype="multipart/form-data">
            ...
            <input type="text" name="title" value="<?= htmlspecialchars((string)($job['title'] ?? '')) ?>" maxlength="120">
            ...
            <label>Област:</label>
            <input type="text" name="region" value="<?= htmlspecialchars((string)($job['region'] ?? '')) ?>">
...
function initMultiUI() {
  ...
  profSearch.addEventListener('input', function () { showSuggestions(profSearch.value); });
...
(function(){
  ...
  var items = [];
  EXISTING.forEach(function(p){ items.push({ kind:'existing', path:p, url:'../' + p }); });
  var coverIndex = 0;
