...
(function () {
    ...
    return fetch('professions_json.php')
      ...
      .then(list => {
        ...
        list.forEach(item => {
          ...
          // item.label е на български; item.count (ако се връща) добавя визуален hint
          opt.value = item.key;
    ...
    return fetch('professions_json.php?children_of=' + encodeURIComponent(mainKey))
      ...
      .then(list => {
        ...
        list.forEach(item => {
          const opt = document.createElement('option');
          opt.value = item.key;
          opt.textContent = item.label; // идва на български
  ...
  /* ---------------- събития ---------------- */
  // Подпрофесии се зареждат, но не „подаваме“ докато не се натисне Приложи
