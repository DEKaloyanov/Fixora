...
<div class="job-details">
    ...
    if (isset($_SESSION['user']) && $_SESSION['user']['id'] !== $job['user_id']) {
        ...
        if (!$existing) {
            // Ако няма заявка – показваме бутон за изпращане
            ...
            // Ако вече има заявка или връзка – бутон за чат
            echo '<a href="chat.php?with=' . $job['user_id'] . '&job=' . $job['id'] . '" 
                    style="display:inline-block;margin-top:20px;padding:10px 20px;background:#002147;color:white;border-radius:5px;text-decoration:none;">
                    💬 Пиши ми (чат)
                </a>';
        }
